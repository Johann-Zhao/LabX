...
- [x] 多模态升级 + 资料上传闭环：①LLM 模型改 `deepseek-v4-flash-vision-exp`，`llm.chat_with_image` 走 OpenAI vision 消息格式（复用重试/MOCK 兜底）；②新增 `file_parser.py`（图片 base64 / PDF pypdf / Word python-docx / TXT 直读，10MB 上限）；③db 新增 `Upload` 表（pending/approved/rejected），`backend/uploads/` 存文件（已 gitignore）；④接口 `POST /api/uploads`（multipart）、`GET /api/uploads`、`POST /api/uploads/{id}/review`，错误码 1009 登记 API.md 第 11 节；⑤审核通过自动生成 `KC-UPLOAD-xxxx` tip 卡片并 `rag.add_card` 同步向量库；⑥新增第四个 MCP Server `file_server.py`（file-mcp，5 工具：解析/待审列表/详情/通过入库/驳回）；⑦orchestrator 全链路支持 `file_context`——图片跳检索阶梯直走 vision，文本截断 1500 字注入问答；⑧前端：AskPage 输入栏左侧回形针上传按钮（预览条+附件气泡）、物料详情页"确认借用/上传资料/问问AI"三按钮+上传弹窗、管理台侧栏"06 资料审核"（pending 角标+通过/驳回+驳回理由弹窗）；⑨上传与审核通过均返回鼓励文案；⑩逻辑修正：对话附件走 `purpose=chat` 只解析不落库（不进资料审核队列、不弹投稿感谢），带附件提问由 `_file_answer` 跳过意图分类与澄清直接识别回答（"这是什么"+图片不再被反问"哪个物料"），物料详情页"上传资料"仍走 `purpose=review` 审核队列；⑪对话内投稿闭环：带附件说"上传/投稿"→ `_contribute` 提炼后直接进审核队列（支持目录外物料，`Upload.material_label` 记录如 RV1126B）；答后邀稿——文本附件知识库未覆盖时回答带 `upload_offer`，前端"投稿到知识库/不用了"按钮确认后以 review 重新上传；投稿入库前 LLM 结构化提炼（标题/关键词/要点/适用场景，保留硬信息剔除冗余，图片走 vision），提炼稿即管理端预览与卡片正文（标题取提炼稿首行）。后端 Python requests 实测上传/审核/入卡全链路通过，Playwright 验证三端 UI（截图存 `deta/shots/`，未入库）
